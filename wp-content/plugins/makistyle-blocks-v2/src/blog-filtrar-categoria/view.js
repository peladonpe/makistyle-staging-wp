...
const $lanzadorModalFiltrar = document.querySelector(
	'#lanzador-modal-filtrar'
);
...
document.addEventListener( 'DOMContentLoaded', function () {
	...
	//   }
	if ( $modalTipoRecurso ) {
		const categoriaName = $selectCategoria.getAttribute(
			'data-categoria-name'
		);
		const categoriaValue = $selectCategoria.getAttribute(
			'data-categoria-value'
		);
		let categoriaSlug = $selectCategoria.getAttribute(
			'data-categoria-slug'
		);
		...
		function setValorLanzadorFiltrar() {
			...
			$enlaceBtnResetFiltro.href = `${ homeUrl }/blog`;
		...
		const $btnCerrarModal = $modalTipoRecurso.querySelector(
			'#boton-cerrar-modal'
		);
		const $btnCancelarFiltro = document.querySelector(
			'#btn-cancelar-filtro'
		);
		...
		$formularioFiltro.addEventListener( 'submit', function ( event ) {
			...
			location.href = `${ homeUrl }/categoria/${ categoriaSlug }`;
		} );
	}
