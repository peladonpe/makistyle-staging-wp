...
$localVideoHtml = '';
if ($urlVideoLocal) {
	$autoplayAttr = ($esAutoplay === '1') ? 'autoplay muted' : '';
	$localVideoHtml = sprintf(
		'<figure class="mkv2-yt-fallback">
			<video width="400" height="%1$s" controls preload="metadata" class="mkv2-local-video" %4$s>
				<source src="%2$s" type="video/mp4">
				%3$s
			</video>
		</figure>',
		$height,
		esc_url($urlVideoLocal),
		__('Tu navegador no soporta el elemento de video.', 'makistyle'),
		$autoplayAttr
	);
}
...
<div <?php echo get_block_wrapper_attributes(); ?>>
	...
	<?php elseif ($urlVideoLocal): ?>
		<div class="mkv2-yt-wrapper">
			<?php echo $localVideoHtml; ?>
			<?php if ($esAutoplay === '1') { ?>
				<button class="mkv2-unmute-btn" type="button">
					<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
						<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon>
