...
function makistyle_get_woocommerce_products_options()
{
	...
	if (!empty($products) && !is_wp_error($products)) {
		foreach ($products as $product) {
			...
			// 	$options_con_fecha[] = [
			// 		'id'          => $product->ID,
			// 		'title'       => $product->post_title . ' (' . $fecha_formateada . ')',
		...
		// Ordenar productos con fecha (de más reciente a más antiguo)
		usort($options_con_fecha, function ($a, $b) {
			return $b['lanzamiento'] <=> $a['lanzamiento'];
		});

		// Ordenar productos sin fecha alfabéticamente
		usort($options_sin_fecha, function ($a, $b) {
			return strcmp($a['title'], $b['title']);
		});
	}

	$options = [];
	foreach ($options_con_fecha as $item) {
		$options[$item['id']] = $item['title'];
	}
	foreach ($options_sin_fecha as $item) {
		$options[$item['id']] = $item['title'];
	}
...
function makistyle_wc_descuento_activo_script()
{
	...
	$is_tax = $screen && $screen->taxonomy === 'descuentos_taxonomia';
	$is_page = $screen && strpos($screen->id, 'page_descuentos2') !== false;
	$is_coupon = $screen && $screen->post_type === 'shop_coupon';

	if (!$is_tax && !$is_page && !$is_coupon) {
		return;
	}?>
	<script type="text/javascript">
		document.addEventListener('DOMContentLoaded', function() {
			...
			// Ejecutar cuando cambie el checkbox, usando blur para evitar retraso visual del check
...
/**
 * Registra el metabox de CMB2 para cupones de WooCommerce (shop_coupon).
 * Añade el checkbox 'Es descuento' para identificar descuentos especiales.
 */
function makistyle_cmb2_cupones()
{
