...
/** Lógica de administración para la tienda personalizada (tienda_pt) */

// Añadir columna de información a la tienda personalizada
add_filter('manage_tienda_pt_posts_columns', 'makistyle_add_post_info_column');

function makistyle_show_tienda_post_info_column($column_name, $post_id)
{
	if ($column_name === 'post_info') {
		...
		$msgPrecio = '<p style="margin: 5px 0; color: #3180FC;"><strong>PRECIO:</strong> ';
		$msgPrecio .= ($precio || $precio == 0)
...
add_action(
	...
	'makistyle_show_tienda_post_info_column',
	10,
	2,
);
...
function makistyle_show_tienda_fecha_lanzamiento_column($column_name, $post_id)
{
...
add_action(
	'manage_tienda_pt_posts_custom_column',
	'makistyle_show_tienda_fecha_lanzamiento_column',
	10,
	2,
);

function makistyle_sortable_tienda_fecha_lanzamiento_column($sortable_columns)
{
...
add_filter(
	...
	'makistyle_sortable_tienda_fecha_lanzamiento_column',
);

function makistyle_orderby_tienda_fecha_lanzamiento($query)
{
...
add_action('pre_get_posts', 'makistyle_orderby_tienda_fecha_lanzamiento');
